      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-2 gap-y-10 md:grid-cols-4 md:gap-0">
        {POINTS.map((point, i) => (
          <Reveal key={point.label} delay={i * 0.06}>
            <div className="relative pr-6 md:border-l md:border-gold/15 md:px-8 md:first:border-l-0 md:first:pl-0">
              <p className="font-display text-3xl text-gold md:text-4xl">{point.value}</p>
              <p className="mt-2 max-w-[18ch] text-xs uppercase leading-snug tracking-[0.12em] text-bone/50">
                {point.label}
              </p>
            </div>
          </Reveal>
